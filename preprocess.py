...
def split_text(text, chunk_size=2000, chunk_overlap=50):
    """
    将文本分割成指定大小的块，并带有重叠。
...
# --- 配置 ---
html_directory = './leuka_data/html' # **** 修改为你的 HTML 文件夹路径 ****
output_json_path = './data/processed_data.json' # **** 输出 JSON 文件路径 ****
CHUNK_SIZE = 2048  # 每个文本块的目标大小（字符数）
...
MEDICAL_KEYWORDS = {
    "白血病", "癌", "疗", "骨髓","移植", "细胞", "血", 
    "蛋白", "治疗", "免疫", "诊", "流感",
    "药物", "临床", "试验", "患", "医","病","症","药","治","法","学","研","淋巴","炎"
}
...
for filename in html_files:
    ...
    if main_text:
        # --- 新增：检查是否与医学相关 ---
        # 标题或正文中包含任意医学关键词则保留
        title_lower = title.lower()  # 统一转小写（可选）
        text_lower = main_text.lower()
        is_medical = any(kw.lower() in title_lower or kw.lower() in text_lower for kw in MEDICAL_KEYWORDS)
        
        if not is_medical:
            print(f"    警告：文件 {filename} 非医学相关，已跳过。")
            continue  # 跳过非医学文章

        # --- 新增：检查正文长度是否小于200 ---
        if len(main_text) < 200:
            print(f"    警告：文件 {filename} 正文内容过短（少于200字），已跳过。")
            continue  # 跳过短内容文章

        # --- 新增：检查是否包含禁止关键词（日记、故事） ---
        forbidden_keywords = {
            "日记", "故事","回忆","让心永远向阳吧","只有经历过的人","我的一路如此之无厘头",
            "护士","来之不易","小编","你必须知道的10"}
        title_contains = any(kw in title for kw in forbidden_keywords)
        text_contains = any(kw in main_text for kw in forbidden_keywords)
        if title_contains or text_contains:
            print(f"    警告：文件 {filename} 包含禁止关键词，已跳过。")
            continue  # 跳过包含禁止关键词的文章
...
print(f"\n处理完成。共处理 {file_count} 个文件，取其中{file_access_count}个文件，生成 {chunk_count} 个文本块。")
